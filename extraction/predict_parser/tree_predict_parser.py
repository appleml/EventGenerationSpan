...
def add_space(text):
    ...
    """
    new_text_list = list()
    # print(split_bracket.findall(text))
    # print(split_bracket.split(text))
    # print(split_bracket.split(text)[1:])
...
class TreePredictParser(PredictParser):
    ...
    def decode(self, gold_list, pred_list, text_list=None, raw_list=None) -> Tuple[List[Dict], Counter]:
        ...
        for gold, pred, text, raw_data in zip(gold_list, pred_list, text_list, raw_list):
            gold = convert_bracket(gold) # 将 <extra_id_0> 等四个符号替换成 左括号， 右括号两个字符
    ...
    def get_event_list(self, tree, text=None):
        ...
        for event_tree in tree: # tree是事件的集合, event_tree是其中一个事件
