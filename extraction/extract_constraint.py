...
class ConstraintDecoder:
    def __init__(self, tokenizer, source_prefix):
        ...
        self.source_prefix_tokenized = tokenizer.encode(source_prefix, add_special_tokens=False) if source_prefix else []
    ...
    def constraint_decoding(self, src_sentence, tgt_generated):
        ...
        if debug:
            print('========================================')
            print('valid tokens:', self.tokenizer.convert_ids_to_tokens(valid_token_ids), valid_token_ids)
            if debug_step:
                input()
...
class TreeConstraintDecoder(ConstraintDecoder):
    def __init__(self, tokenizer, type_schema, *args, **kwargs):
        ...
        self.type_start = self.tokenizer.convert_tokens_to_ids([type_start])[0] # 32099
        self.type_end = self.tokenizer.convert_tokens_to_ids([type_end])[0] #32038
    ...
    def check_state(self, tgt_generated):
        ...
        special_index_token = list(filter(lambda x: x[1] in special_token_set, list(enumerate(tgt_generated)))) # 特殊字符在生成序列中的位置
        ...
        if start_number == end_number + 1: # 少一个end 符号
            state = 'start_first_generation'
        elif start_number == end_number + 2: # 相比 start符号， 少了两个end符号
    ...
    # generated 从上一个特殊符号开始的第1个字符
    def search_prefix_tree_and_sequence(self, generated: List[str], prefix_tree: Dict, src_sentence: List[str], end_sequence_search_tokens: List[str] = None):
        """
        ...
        :return:
        """
        tree = prefix_tree # dict的value还是dict
        for index, token in enumerate(generated):
            tree = tree[token]
            is_tree_end = (len(tree) == 1 and self.tree_end in tree)

            if is_tree_end: # trigger 类型已经识别完成， 要开始识别trigger_name了
        ...
        valid_token = list(tree.keys()) # trigger 的类型还没有遍历， 还需要继续遍历trigger类型的子词
        return valid_token
    ...
    def get_state_valid_tokens(self, src_sentence, tgt_generated):
        ...
        if self.tokenizer.eos_token_id in src_sentence:
            src_sentence = src_sentence[:src_sentence.index(self.tokenizer.eos_token_id)] # 去掉end_token_id

        state, index = self.check_state(tgt_generated) # index = last_special_index
        ...
        elif state == 'start_first_generation': # （）生成一个空
