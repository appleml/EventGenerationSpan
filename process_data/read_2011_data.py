...
转换成 
'''
def read_data(data_path, genia_parse_path, output_path, output_path_span):
    ...
    for file_name in files:
        ...
        with open(data_path + file_name, "r") as data_fp, open(genia_parse_path + file_name, "r") as genia_parse_fp, \
                open(output_path + new_filename, "w") as writer, open(output_path_span+new_filename, "w") as span_writer:
            old_sent = ""
            sent_idx = 0
            str_prot_set = []
            str_prot_idxs = []
            ...
            for line in data_fp:
                ...
                if not len(line):
                    ...
                    if event_tree != "<extra_id_0> <extra_id_1>":
                        ...
                        ouput_length_set.append(len(event_tree.split()))

                    if len(event_tree.split()) > max_output_length:
                        max_output_length = len(event_tree.split())
    ...
    print("输出的最大的句子长度：", max_output_length)
    print("平均输出句子长度：", sum(ouput_length_set)//has_event_sent_num)
        ...
        with open(data_path + file_name, "r") as data_fp, open(parse_path + file_name, "r") as parse_fp, open(output_path+new_filename, "w") as writer:
            ...
            for line in data_fp:
                ...
                if not len(line):
                    ...
                    old_sent = line
                    sent_idx += 1
    print("test中句子个数：", total_sent_num)
...
if __name__ == '__main__':
    ...
    train_processed = "data/text2tree/2011_data/2011_train/" # 转换为增广后的数据
    span_train_processed = "data/text2tree/2011_data_span/2011_train/"
    ...
    devel_processed = "data/text2tree/2011_data/2011_devel/"
    span_devel_processed = "data/text2tree/2011_data_span/2011_devel/"
    read_data(abs_path+devel_data, abs_path+devel_genia, abs_path+devel_processed, abs_path+span_devel_processed)
    print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")

    test_data = "data/raw_data/2011_data_one/2011_test/"
    test_genia = "data/raw_data/2011_genia_parse_one_split_bio/2011_test/"
    test_processed = "data/text2tree/2011_data/2011_test/"
