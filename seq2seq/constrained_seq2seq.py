...
class ConstraintSeq2SeqTrainingArguments(Seq2SeqTrainingArguments):
    ...
    label_smoothing_sum: bool = field(default=False, metadata={"help": "Whether to use sum token loss for label smoothing"})
...
class ConstraintSeq2SeqTrainer(Seq2SeqTrainer):
    def __init__(self, decoding_type_schema=None, decoding_format='tree', source_prefix=None, *args, **kwargs):
        ...
        if self.args.constraint_decoding: # 其实将 constraint_decoding 增加到 TraingArgument中，这样做的目的是传值方便
    ...
    def prediction_step(
        ...
        def prefix_allowed_tokens_fn(batch_id, sent): # the previously generated tokens :obj:`inputs_ids`, sent是生成的部分字符串
