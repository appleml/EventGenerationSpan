...
'''
两种数据处理方式（1）将实体的识别和关系抽取作为一个整体， 输出的序列上包含实体和关系的信息
比较麻烦的是一个论元可能是多个trigger的论元，顺序问题
...
def read_data(input_path, genia_parse_path, output_path):
    ...
    for file_name in files:
        augmented_sentences = []
        with open(input_path + "/" + file_name, "r") as data_fp, open(genia_parse_path + "/" + file_name, "r") as genia_parse_fp, open(output_path+"/"+file_name, "w") as write_fp:
            old_sent = ""
            str_prot_set = []
            str_prot_idxs = []
            str_jprot_set = []
