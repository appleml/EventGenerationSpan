...
class Text2Tree(TargetFormat):
    ...
    def annotate_predicate_arguments(tokens, predicate_arguments, mark_tree=False, multi_tree=False, zh=False):
        ...
        for predicate_argument in predicate_arguments:
            ...
            role_str_list = list()
            for role_name, role_tokens in predicate_argument['arguments']: # arguments += [[argument['role'], list(range(argument_entity['start'], argument_entity['end']))]]
                # if role_name == 'Place' or role_name.startswith('Time'):
                if role_name == event_type:
                    continue

                role_text = get_str_from_tokens(role_tokens, tokens, separator=token_separator) # 论元名称
                if mark_tree:
                    role_str = ' '.join([role_start, role_name, role_text, role_end])
                else:
                    role_str = ' '.join([type_start, role_name, role_text, type_end]) # '<extra_id_0>' role_name role_text '<extra_id_1>'”
