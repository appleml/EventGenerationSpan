...
def data_counter_to_table(data_counter):
    ...
    for filename, file_counter in data_counter.items():
        table += [[filename, file_counter['sentence'], file_counter['event'], file_counter['argument']]]
...
def output_schema(event_schema_set, output_file):
    ...
    # key 是 eventType, value是set, 存储的是roleType
    ...
    for event_type in event_type_set_dict:
        event_type_list_dict[event_type] = list(event_type_set_dict[event_type])
